...
cdef extern from "scs.h":
    # these typedefs should depend on what SCS actually built
    ctypedef np.float64_t scs_float
    ctypedef np.int64_t scs_int
...
def mytest3():
    ij = np.array([[0,1,2,3],[0,1,2,3]], dtype=np.int64)
    A = sp.csc_matrix(([-1.,-1.,1.,1.], ij), (4,4))
    cdef np.ndarray[scs_float] b = np.array([0.,0.,1,1])
    cdef np.ndarray[scs_float] c = np.array([1.,1.,-1,-1])
    cone = {'l':4}
